...
async def validate_username(
    payload: ValidateUsername, db: AsyncSession = Depends(get_db)
):
    try:
...
async def request_registration_otp(
    ...
    if existing_user:
        try:
            await send_email(payload.email, "Password Reset", f"Your OTP is {otp}")
        except Exception:
...
async def register_student(payload: UserRegister, db: AsyncSession = Depends(get_db)):
    try:
        ...
        if not db_record:
            raise HTTPException(
                status_code=400,
                detail="Expired or non-existent OTP. Please request a new one.",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="Student batch must be a valid number string (e.g., '2026')",
            )
        ...
        student_result = await db.execute(
            ...
            """),
            {"id": user_id, "student_batch": batch_integer},
        )
        ...
        profile_image_url = (
            settings.default_profile_image_url if "settings" in globals() else None
        )
        ...
        return {
            ...
            "user": {
                ...
                "user_updated_at": new_user["updated_at"].isoformat()
                if new_user["updated_at"]
                else None,
                "student_updated_at": new_student["updated_at"].isoformat()
                if new_student["updated_at"]
                else None,
            },
        }
