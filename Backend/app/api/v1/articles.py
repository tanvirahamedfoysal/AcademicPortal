...
async def get_article(
    ...
    try:
        ...
        """)
        result = await db.execute(query, {"id": article_id, "user_uuid": auth["data"]["uuid"]})
        article = result.mappings().first()
...
async def create_article(
    ...
    try:
        ...
        await db.commit()
        return {"message": "Article created successfully", "data": result.mappings().first()}
...
async def update_article_status(
    ...
    try:
        ...
        published_clause = "published_at = NOW()," if payload.status.upper() == 'PUBLISHED' else ""
        ...
        return {"message": f"Article status updated to {payload.status}", "data": updated_article}
    except HTTPException:
        raise
    except SQLAlchemyError:
        await db.rollback()
        raise HTTPException(status_code=500, detail="Database error occurred. Verify status value is valid.")
...
async def get_article_last_update(
    article_id: str,
    db: AsyncSession = Depends(get_db)
):
    try:
