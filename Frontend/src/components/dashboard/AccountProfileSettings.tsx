      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-6 bg-[linear-gradient(130deg,#0b2823_0%,#0f3b34_62%,#174b3f_100%)] px-6 py-8 text-white md:grid-cols-[auto_1fr_auto] md:items-center md:px-8">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-emerald-50/75">Keep your account identity, public About Me description, profile image, and security information current.</p>
          </div>
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            ...
            <div className="md:col-span-2 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <label className={`inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-semibold text-[#0f3b34] shadow-sm ring-1 ring-slate-200 transition hover:ring-emerald-700/30 ${photoUploading ? 'pointer-events-none opacity-60' : ''}`}>{photoUploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />} Upload profile photo<input type="file" accept="image/*" className="hidden" onChange={uploadProfilePhoto} disabled={photoUploading} /></label>
              </div>
              <label className="mt-4 block"><span className="mb-2 flex items-center gap-2 text-xs font-semibold text-slate-600"><Camera className="h-3.5 w-3.5" /> Profile image URL</span><input value={form.image_url} onChange={(e) => setForm({ ...form, image_url: e.target.value })} placeholder="https://…" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/10" /></label>
            </div>
