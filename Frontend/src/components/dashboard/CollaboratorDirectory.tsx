    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-6 bg-[linear-gradient(130deg,#fffdfb_0%,#edf6ff_52%,#dff7f6_100%)] px-6 py-8 text-slate-900 md:grid-cols-[1fr_auto] md:items-end md:px-8">
          <div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-700"><Users className="h-3.5 w-3.5" /> Research network</div><h1 className="font-serif text-3xl font-semibold md:text-4xl">Collaborators</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">A concise directory of research collaborators connected to the academic portal.</p></div>
          {canManage && <button onClick={openCreate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#5f91a0] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#4f8294]"><Plus className="h-4 w-4" /> Add collaborator</button>}
        </div>
        <div className="border-b border-slate-200 bg-slate-50/70 p-4 md:px-6"><div className="relative max-w-md"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search collaborators" className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm outline-none focus:border-[#78bac5]" /></div></div>
        {feedback && <div className="border-b border-slate-200 bg-[#edf6ff] px-6 py-3 text-sm text-slate-700">{feedback}</div>}
        {loading ? <div className="flex min-h-60 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-[#5f91a0]" /></div> : filtered.length === 0 ? <div className="px-6 py-16 text-center text-sm text-slate-500">No collaborators match your search.</div> : <div className="grid gap-px bg-slate-100 md:grid-cols-2 xl:grid-cols-3">{filtered.map((collaborator) => <article key={collaborator.uuid} className="bg-white p-6"><div className="flex items-start gap-4">{collaborator.image_url ? <img src={collaborator.image_url} alt="" className="h-16 w-16 rounded-2xl object-cover" /> : <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#dff7f6] font-serif text-xl text-[#5f91a0]">{collaborator.name.charAt(0)}</div>}<div className="min-w-0 flex-1"><div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#527f8f]">Research collaborator</div><h2 className="mt-1 truncate font-serif text-xl font-semibold text-slate-900">{collaborator.name}</h2></div></div>{canManage && <div className="mt-5 flex gap-2 border-t border-slate-100 pt-4"><button onClick={() => openEdit(collaborator)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 hover:text-[#5f91a0]"><Edit3 className="h-4 w-4" /> Edit</button><button onClick={() => remove(collaborator)} disabled={workingId === collaborator.uuid} className="inline-flex items-center gap-2 rounded-xl border border-[#cde3ea] px-3 py-2 text-sm font-medium text-[#527f8f] hover:bg-[#edf6ff] disabled:opacity-50">{workingId === collaborator.uuid ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} Remove</button></div>}</article>)}</div>}
      </section>

      {modalOpen && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/35 p-0 backdrop-blur-sm sm:items-center sm:p-4"><div className="max-h-[92dvh] w-full max-w-2xl overflow-y-auto rounded-t-[1.75rem] bg-white shadow-2xl sm:rounded-[1.75rem]"><div className="flex items-center justify-between border-b border-slate-200 px-4 py-4 sm:px-6 sm:py-5"><div><div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#5f91a0]">Collaboration record</div><h2 className="font-serif text-2xl font-semibold text-slate-900">{editing ? 'Update collaborator' : 'Add collaborator'}</h2></div><button onClick={() => setModalOpen(false)} className="rounded-full border border-slate-200 p-2 text-slate-500"><X className="h-4 w-4" /></button></div><form onSubmit={save} className="grid gap-5 p-6 md:grid-cols-2"><label className="block"><span className="mb-2 block text-sm font-semibold text-slate-700">Name</span><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#78bac5]" /></label><label className="block"><span className="mb-2 block text-sm font-semibold text-slate-700">Organization</span><input value={form.organization} onChange={(e) => setForm({ ...form, organization: e.target.value })} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#78bac5]" /></label><label className="block md:col-span-2"><span className="mb-2 block text-sm font-semibold text-slate-700">Bio {editing && <span className="font-normal text-slate-400">(leave blank to keep existing)</span>}</span><textarea required={!editing} rows={4} value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#78bac5]" /></label><label className="block"><span className="mb-2 block text-sm font-semibold text-slate-700">Website URL</span><input value={form.website_url} onChange={(e) => setForm({ ...form, website_url: e.target.value })} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#78bac5]" /></label><label className="block"><span className="mb-2 block text-sm font-semibold text-slate-700">Image URL</span><input value={form.image_url} onChange={(e) => setForm({ ...form, image_url: e.target.value })} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#78bac5]" /></label><div className="md:col-span-2 flex justify-stretch sm:justify-end"><button disabled={saving} type="submit" className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#5f91a0] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#4f8294] disabled:opacity-60 sm:w-auto">{saving && <Loader2 className="h-4 w-4 animate-spin" />}{editing ? 'Save changes' : 'Add collaborator'}</button></div></form></div></div>}
    </div>
  );
}
