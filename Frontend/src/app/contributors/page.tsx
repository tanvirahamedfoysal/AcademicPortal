            ...
            {students.map((student) => (
              <article key={student.uuid} className="academic-card p-6">
                ...
                <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4 text-xs font-semibold text-slate-500">
                  ...
                  <span className="rounded-full bg-[#dff7f6] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[#5f91a0]">{student.status || 'Student'}</span>
                </div>
              </article>
            ))}
          </div>
        ) : (
          <div className="academic-card px-6 py-16 text-center"><UsersRound className="mx-auto h-8 w-8 text-[#689aa6]" /><h2 className="mt-5 font-serif text-2xl font-bold">Community profiles will appear here</h2><p className="mt-3 text-sm text-slate-500">Active student records from the existing API populate this directory automatically.</p></div>
        )}
      </section>
    </MainLayout>
  );
