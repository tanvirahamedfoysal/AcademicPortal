    ...
    <MainLayout>
      <section className="border-b border-slate-200 bg-[#fbfcfa]">
        <div className="page-shell py-16 lg:py-20">
          <div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <div className="rounded-2xl border border-emerald-950/10 bg-[#e7efe9] px-5 py-4">
              <p className="text-xs font-semibold uppercase tracking-[0.15em] text-emerald-800">Published works</p>
              <p className="mt-1 font-serif text-3xl font-bold text-[#0f3b34]">{articles.length}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="page-shell py-14 lg:py-20">
        <div className="mb-8 flex items-center justify-between gap-4 border-b border-slate-200 pb-5">
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-600"><BookOpenText className="h-4 w-4 text-emerald-700" /> Research archive</div>
        ...
        {articles.length > 0 ? (
          <div className="grid gap-4">
            {articles.map((article, index) => (
              <Link key={article.article_uuid} href={`/articles/${article.article_uuid}`} className="group academic-card grid gap-5 p-6 transition duration-300 hover:-translate-y-0.5 hover:border-emerald-900/20 hover:shadow-[0_18px_50px_rgba(15,35,30,0.08)] sm:grid-cols-[70px_1fr_auto] sm:items-center sm:p-7">
                ...
                <div>
                  <div className="mb-3 flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500">
                    <span className="inline-flex items-center gap-1.5 text-emerald-800"><CalendarDays className="h-3.5 w-3.5" /> {formatDate(article.published_at)}</span>
                    <span>•</span>
                    <span>Research article</span>
                  </div>
                  <h2 className="max-w-4xl font-serif text-2xl font-bold leading-8 tracking-[-0.02em] text-slate-950 group-hover:text-[#0f3b34]">{article.article_title}</h2>
                  <p className="mt-3 text-sm leading-6 text-slate-500">Open the full publication record and article body.</p>
                </div>
                <span className="grid h-11 w-11 place-items-center rounded-full border border-slate-200 bg-slate-50 text-slate-500 transition group-hover:bg-[#0f3b34] group-hover:text-white"><ArrowUpRight className="h-4 w-4" /></span>
              </Link>
            ))}
          </div>
        ) : (
          <div className="academic-card px-6 py-16 text-center">
            <span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#e7efe9] text-[#0f3b34]"><FileText className="h-6 w-6" /></span>
