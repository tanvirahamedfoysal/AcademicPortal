  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
        ...
        <div className="border-b border-slate-200 bg-slate-50/70 p-4 md:px-6"><div className="relative max-w-md"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search students or batches" className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm outline-none focus:border-[#78bac5]" /></div></div>
        {loading ? <div className="flex min-h-60 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-[#5f91a0]" /></div> : filtered.length === 0 ? <div className="px-6 py-16 text-center text-sm text-slate-500">No active students match your search.</div> : <div className="grid gap-px bg-slate-100 md:grid-cols-2 xl:grid-cols-3">{filtered.map((student) => <article key={student.uuid} className="bg-white p-6"><div className="flex items-center gap-4">{student.image_url ? <img src={student.image_url} alt="" className="h-14 w-14 rounded-2xl object-cover" /> : <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#dff7f6] font-serif text-xl text-[#5f91a0]">{student.name?.charAt(0) || 'S'}</div>}<div className="min-w-0"><h2 className="truncate font-serif text-lg font-semibold text-slate-900">{student.name || student.username || 'Student'}</h2><p className="truncate text-sm text-slate-500">{student.username ? `@${student.username}` : 'Research community member'}</p></div></div><div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">{student.student_batch ? `Batch ${student.student_batch}` : 'Active student'}</span><a href={`mailto:${student.email}`} className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:text-[#5f91a0]" aria-label={`Email ${student.name}`}><Mail className="h-4 w-4" /></a></div></article>)}</div>}
      </section>
    </div>
  );
}
