...
export default async function RepositoriesPage() {
  ...
  return (
    <MainLayout>
      <section className="bg-[#0b2823] text-white">
        <div className="page-shell grid gap-10 py-16 lg:grid-cols-[1fr_.45fr] lg:items-end lg:py-20">
          <div className="max-w-4xl">
            <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#e6c27a]">Open knowledge</p>
            <h1 className="mt-4 font-serif text-5xl font-bold tracking-[-0.045em] sm:text-6xl">Research resources & repository</h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-emerald-50/70">A shared archive of documents and academic material supporting research, teaching, and independent learning.</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[.06] p-5">
            <div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-[#e6c27a]" /><p className="text-sm font-semibold">Backend-managed document library</p></div>
            <p className="mt-2 text-xs leading-6 text-emerald-50/55">Files shown here come directly from the existing repository API.</p>
          </div>
        </div>
      </section>
        ...
        {documents.length ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {documents.map((document) => (
              <a key={String(document.id)} href={document.url} target="_blank" rel="noreferrer" className="group academic-card flex min-h-[220px] flex-col justify-between p-6 transition duration-300 hover:-translate-y-1 hover:border-emerald-900/20 hover:shadow-[0_18px_50px_rgba(15,35,30,0.08)]">
                <div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#e7efe9] text-[#0f3b34]"><FileText className="h-5 w-5" /></span>
                ...
                <div className="mt-7 flex items-center justify-between border-t border-slate-100 pt-4 text-xs font-bold text-emerald-800">
          ...
          <div className="academic-card px-6 py-16 text-center">
            <span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#e7efe9] text-[#0f3b34]"><FolderOpen className="h-6 w-6" /></span>
