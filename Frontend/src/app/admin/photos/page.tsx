    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      ...
      <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        <div className="flex flex-col justify-between gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-center">
          ...
          <button type="button" onClick={saveGalleryDetails} disabled={saving || media.gallery.length === 0} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#5f91a0] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#4f8294] disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save gallery details</button>
        </div>
      ...
      <p className="text-xs leading-5 text-slate-400">Portfolio and gallery photos saved here are used on the public portfolio page.</p>
    </div>
  );
}
