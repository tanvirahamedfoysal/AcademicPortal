    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-6 bg-[linear-gradient(130deg,#fffdfb_0%,#edf6ff_52%,#dff7f6_100%)] px-6 py-8 text-slate-900 md:grid-cols-[1fr_auto] md:items-end md:px-8">
          ...
          <button onClick={openPromotion} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#5f91a0] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#4f8294]"><ArrowUpCircle className="h-4 w-4" /> Promote user</button>
        </div>
        ...
        {loading ? <div className="flex min-h-60 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-[#5f91a0]" /></div> : filtered.length === 0 ? <div className="px-6 py-16 text-center text-sm text-slate-500"><Users className="mx-auto mb-3 h-9 w-9 text-slate-300" />No moderators found.</div> : <div className="divide-y divide-slate-100">{filtered.map((moderator) => <article key={moderator.uuid} className="grid gap-4 px-4 py-4 sm:px-6 sm:py-5 md:grid-cols-[1fr_auto] md:items-center"><div><h2 className="font-serif text-lg font-semibold text-slate-900">{moderator.name || 'Portal Moderator'}</h2><p className="mt-1 text-sm text-slate-500">{moderator.email}{moderator.mobile_number ? ` · ${moderator.mobile_number}` : ''}</p><span className="mt-2 inline-flex rounded-full bg-[#dff7f6] px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#689aa6]">{moderator.status}</span></div><button onClick={() => demote(moderator)} disabled={workingId === moderator.uuid} className="inline-flex items-center gap-2 rounded-xl border border-[#cde3ea] bg-white px-3.5 py-2 text-sm font-semibold text-[#527f8f] transition hover:bg-[#edf6ff] disabled:opacity-50">{workingId === moderator.uuid ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} Remove role</button></article>)}</div>}
      </section>

      {modalOpen && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/35 p-0 backdrop-blur-sm sm:items-center sm:p-4"><div className="max-h-[92dvh] w-full max-w-2xl overflow-hidden rounded-t-[1.75rem] bg-white shadow-2xl sm:rounded-[1.75rem]"><div className="flex items-center justify-between border-b border-slate-200 px-4 py-4 sm:px-6 sm:py-5"><div><div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#5f91a0]">Existing verified users</div><h2 className="font-serif text-2xl font-semibold text-slate-900">Promote to moderator</h2></div><button onClick={() => setModalOpen(false)} className="rounded-full border border-slate-200 p-2 text-slate-500"><X className="h-4 w-4" /></button></div><div className="p-5"><div className="relative"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={studentQuery} onChange={(e) => setStudentQuery(e.target.value)} placeholder="Search students" className="w-full rounded-xl border border-slate-200 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-[#78bac5]" /></div></div><div className="max-h-[56vh] divide-y divide-slate-100 overflow-y-auto">{candidateStudents.length === 0 ? <div className="px-6 py-12 text-center text-sm text-slate-500">No eligible users found.</div> : candidateStudents.map((student) => <div key={student.uuid} className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div className="min-w-0"><div className="truncate font-medium text-slate-900">{student.name || student.username || 'Student'}</div><div className="truncate text-sm text-slate-500">{student.email}{student.student_batch ? ` · Batch ${student.student_batch}` : ''}</div></div><button onClick={() => promote(student)} disabled={workingId === student.uuid} className="inline-flex min-h-11 w-full shrink-0 items-center justify-center gap-2 rounded-xl bg-[#5f91a0] px-3.5 py-2 text-sm font-semibold text-white transition hover:bg-[#4f8294] disabled:opacity-50 sm:w-auto">{workingId === student.uuid ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowUpCircle className="h-4 w-4" />} Promote</button></div>)}</div></div></div>}
    </div>
  );
}
