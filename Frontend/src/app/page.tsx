    ...
    <MainLayout>
        ...
        <div className="grid gap-10 lg:grid-cols-[.84fr_1.16fr] lg:gap-16">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-2.5">
              {(interests.length ? interests : ['Research methods', 'Academic teaching', 'Evidence-led inquiry', 'Open scholarship']).slice(0, 10).map((interest) => (
                <span key={interest} className="rounded-full border border-[#a9d7df]/10 bg-[#dff7f6] px-4 py-2 text-xs font-semibold text-[#5e8997]">{interest}</span>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="academic-card p-7 sm:p-8">
              <div className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#9f5f7a]"><Sparkles className="h-3.5 w-3.5" /> Research practice</div>
            ...
            <div className="academic-card p-7 sm:p-8">
              <Quote className="h-8 w-8 text-[#b96586]" />
      ...
      <section className="border-y border-[#dce7ee] bg-[#fffdfb]">
        <div className="page-shell py-20 lg:py-24">
          <div className="mb-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            ...
            <Link href="/articles" className="inline-flex items-center gap-2 text-sm font-bold text-[#5f91a0] hover:underline">View publication archive <ArrowRight className="h-4 w-4" /></Link>
          </div>
            ...
            {(researcherArticles.length ? researcherArticles.slice(0, 3) : [
              ...
              const content = (
                <article className="academic-card group flex min-h-[280px] flex-col p-7 transition duration-300 hover:-translate-y-1 hover:shadow-[0_22px_60px_rgba(15,35,30,.1)]">
                  <div className="flex items-center justify-between"><span className="font-serif text-2xl font-bold text-slate-300">{String(index + 1).padStart(2, '0')}</span>{!placeholder && <ArrowUpRight className="h-5 w-5 text-slate-400 transition group-hover:text-[#5f91a0]" />}</div>
                  <p className="mt-8 text-[10px] font-bold uppercase tracking-[0.16em] text-[#5f91a0]">{formatDate(article.published_at)}</p>
      ...
      <section className="page-shell py-20 lg:py-28">
        <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
          <div className="overflow-hidden rounded-[32px] border border-[#d4e6ed] bg-[linear-gradient(135deg,#edf6ff_0%,#dff7f6_48%,#f8dce7_100%)] p-8 text-slate-900 shadow-sm sm:p-10 lg:p-12">
            <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#9f5f7a]">Open academic resources</p>
            ...
            <p className="mt-5 max-w-2xl text-base leading-8 text-slate-500">Browse documents and learning material uploaded through the research repository.</p>
            <div className="mt-8 flex flex-wrap gap-3"><Link href="/repositories" className="inline-flex items-center gap-2 rounded-full bg-[#b96586] px-5 py-3 text-sm font-bold text-white">Open repository <ArrowRight className="h-4 w-4" /></Link><Link href="/collaborators" className="inline-flex items-center gap-2 rounded-full border border-[#a9d7df] bg-[#fffdfb]/70 px-5 py-3 text-sm font-bold text-[#527b89]">Research network <Network className="h-4 w-4" /></Link></div>
          </div>
          <div className="academic-card p-8 sm:p-10 lg:p-12">
            ...
            <Link href="/contact" className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#5f91a0] px-5 py-3 text-sm font-bold text-white">Contact {researcherName} <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
      </section>
    </MainLayout>
